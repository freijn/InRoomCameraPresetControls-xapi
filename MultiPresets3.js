/*  Camera Preserts Version 1.3  Frank Reijn  2021-03-18
...
xapi.event.on('UserInterface Extensions Widget Action', (event) => {
  ...
  if(event.WidgetId == 'Focusset'){
    if(event.Type == 'pressed'){
      ...
      camera3active && xapi.command("Camera PositionSet", {CameraId: CAMERAID_CAMERA_THREE,Focus:focusstr});
    }
    else if(event.Type == 'released'){
  ...
  if(event.WidgetId == 'brightset'){
    ...
    //none
      ...
      camera1active && xapi.config.set('Cameras Camera 3 Brightness DefaultLevel',Brightval); 
      // update the value to the Gui
      xapi.Command.UserInterface.Extensions.Widget.SetValue({
        WidgetId: 'brtext',
        Value: Brightval,
  ...
  if(event.WidgetId == 'wbset'){
    if(event.Type == 'pressed'){
      ...
      camera3active && xapi.config.set('Cameras Camera 3 Whitebalance Level',WBval);
      // update the value to the Gui
      xapi.Command.UserInterface.Extensions.Widget.SetValue({
        WidgetId: 'wbtext',
        Value: WBval,
  ...
  if(event.WidgetId == 'NESW'){
    ...
    if(event.WidgetId == 'ultrasonic'){
        if(event.Type == 'changed'){     
          if (event.Value == "on"){
            xapi.config.set('ultrasonic Control:','On');
            xapi.command('Proximity Services Activate');
          } else {
            xapi.config.set('ultrasonic Control:','Off');
            xapi.command('Proximity Services Deactivate');
          }
        }
      } 
